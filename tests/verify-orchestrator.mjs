import assert from "node:assert/strict";
import path from "node:path";
import { spawnSync } from "node:child_process";
import { fileURLToPath } from "node:url";
import { cleanCanonicalOutput } from "./verify-orchestration.mjs";

const root = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..");
const outputRoot = path.join(root, "_site");
const unitTests = Object.freeze([
  "tests/inventory-comparator.test.mjs",
  "tests/git-binary-reader.test.mjs",
  "tests/html-contract.test.mjs",
  "tests/measurement-mode.test.mjs",
  "tests/launch-readiness-36.test.mjs",
  "tests/quote-cta-38.test.mjs",
  "tests/nonvisual-readiness-40.test.mjs",
  "tests/nonvisual-closure-41.test.mjs",
  "tests/legacy-retirement-42r1.test.mjs",
  "tests/clean-images-43.test.mjs",
  "tests/project-proof-ui-44.test.mjs",
  "tests/project-galleries-44r2.test.mjs",
  "tests/site-ui-46.test.mjs",
  "tests/site-wd-delta-47.test.mjs",
  "tests/cta-cards47.test.mjs",
  "tests/whitespace47.test.mjs",
  "tests/utility-cta47.test.mjs",
  "tests/mobile-hover-47r1.test.mjs",
  "tests/input-modality-47r2.test.mjs",
  "tests/editorial-motion-47r3.test.mjs",
  "tests/wd-ecommerce-embed-46.test.mjs",
  "tests/wd-ecommerce-embed-47.test.mjs",
  "tests/wd-static-46.test.mjs",
  "tests/site-footer-46.test.mjs",
  "tests/cookie-runtime-40.test.mjs",
  "tests/lead-form-runtime.test.mjs",
  "tests/site-shell.test.mjs",
  "tests/site-final-polish-33.test.mjs",
  "tests/original-images-31.test.mjs",
  "tests/visual-consistency-32.test.mjs",
  "tests/verify-orchestration.test.mjs",
  "tests/binary-post-build.test.mjs"
]);

function runPhase(label, args) {
  const result = spawnSync(process.execPath, args, {
    cwd: root,
    shell: false,
    stdio: "inherit",
    windowsHide: true
  });

  if (result.error) throw result.error;
  assert.equal(result.status, 0, `${label} failed with exit code ${result.status}`);
  console.log(`VERIFY_ORCHESTRATION_PHASE=${label}:PASS`);
}

cleanCanonicalOutput(root, outputRoot);
console.log("VERIFY_ORCHESTRATION_INITIAL_CLEAN=PASS");
runPhase("OUTPUT_INDEPENDENT_TESTS", ["--test", ...unitTests]);
runPhase("TWO_BUILD_PRODUCT_VERIFICATION", ["tests/verify.mjs"]);
console.log("VERIFY_ORCHESTRATION=PASS");
