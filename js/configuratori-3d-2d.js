...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /* ---------------------------------
   * Carousel con loop infinito (da ref)
   * --------------------------------- */
  ...
  /* ---------------------------------
   * Apri chatbot su click pulsante
   * --------------------------------- */
  const openChatbotBtn = document.getElementById('open-chatbot');
  if (openChatbotBtn) {
    openChatbotBtn.addEventListener('click', () => {
      // Simula apertura chatbot (assumendo che il JS del chatbot esponga una funzione o event)
      const root = document.getElementById('root');
      if (root) {
        root.dispatchEvent(new Event('openChatbot')); // O chiama window.openChatbot() se definito
      } else {
        console.warn('Chatbot non pronto');
      }
    });
  }
});