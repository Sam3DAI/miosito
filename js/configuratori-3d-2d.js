// configuratori-3d-2d.js — build 2025-08-25g (Sam) — AR parity + fix ombra AirPods (Android WebXR)
// - iOS: deep-link senza overlay; bake & swap prima di Quick Look (come prima).
// - Android: overlay solo qui; WebXR diretto. In AR: nasconde in modo deterministico cuffie + ombra se OFF.
// - Tutto il resto invariato rispetto alla build 2025-08-25f.
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (document.getElementById('renderCanvas')) {
    ...
    BABYLON.SceneLoader.ImportMesh('', './assets/', 'iphone_16_pro_configuratore_3d.glb', scene, (meshes) => {
      ...
      // --- VISIBILITÀ CUFFIE + OMBRA in MV (Android/WebXR e iOS viewer interno) ---
      ...
      // Enforce visibilità anche quando parte la sessione AR (Android)
      if (mv) {
        mv.addEventListener('ar-status', (ev) => {
          ...
          if (status === 'session-started') {
            ...
            // applica subito
            setAirpodsVisibleInMV(hpOn);
            // e rinforza per i primi frame (alcuni device ricreano i nodi nei primissimi rAF)
            let frames = 10;
            const tick = () => {
              setAirpodsVisibleInMV(hpOn);
      ...
      if (arButton) {
        arButton.addEventListener('click', async (e) => {
          ...
          try {
            if (IS_ANDROID) {
              // ANDROID: WebXR diretto — prima di aprire AR applico subito il vincolo su cuffie/ombra
              const hpOn = !!document.getElementById('toggle-airpods')?.checked;
              setAirpodsVisibleInMV(hpOn);
              await mv.activateAR(); // reticolo + tap-to-place + drag
            } else {
              // iOS: config + bake & swap PRIMA di Quick Look
      ...
      (function handleDeepLink() {
        ...
        const launchAR = async () => {
          try {
            ...
            if (IS_ANDROID) {
              // Android: serve gesto → overlay; prima garantisco che cuffie/ombra siano coerenti
              const hpOn = !!document.getElementById('toggle-airpods')?.checked;
